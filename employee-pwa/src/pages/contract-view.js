...
let manualZoomScale = 1.0; // 手動ズーム倍率
...
export function renderContractView(sheetId) {
  ...
  return `
    <header class="app-header">
      ...
      <div class="app-header__zoom-controls" style="display: flex; gap: 8px;">
        <button id="zoomOutBtn" class="btn--zoom" style="padding: 4px 10px; background: #334155; color: white; border-radius: 4px; border: none; font-weight: bold;">－</button>
        <button id="zoomInBtn" class="btn--zoom" style="padding: 4px 10px; background: #334155; color: white; border-radius: 4px; border: none; font-weight: bold;">＋</button>
      </div>
    </header>
...
function applyZoom() {
  ...
  if (table && htmlContainer) {
    ...
    const baseScale = tableWidth > containerWidth ? containerWidth / tableWidth : 1;
    const finalScale = baseScale * manualZoomScale;
    ...
    if (previewContainer) {
      ...
      htmlContainer.style.height = `${Math.ceil(tableHeight * finalScale) + 20}px`;
    }
  }
}
